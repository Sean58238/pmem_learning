...
int main(int argc, char *argv[]){
    ...
    /*open src-file*/
    if((srcdf = open(argv[1], O_RDONLY)) < 0)
    {
        perror(argv[1]);
        exit(1); 
    }
    ...
    if((pmemaddr == pmem_map_file(argv[2], BUF_LEN, PMEM_FILE_CREATE|PMEM_FILE_EXCL, 0666, &mapped_len, &is_pmem)))
    {
        perror(argv[1]);
        exit?(1);
    }

    /*read up to BUF_LEN from srcdf*/
